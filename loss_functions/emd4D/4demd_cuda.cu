...
// author: Minghua Liu
// Adapted to 4D by Ben Wagner on 1/28/25
#include <stdio.h>
#include <ATen/ATen.h>
...
__global__ void Bid(int b, int n, const float * xyz1, const float * xyz2, float eps, int * assignment, int * assignment_inv, float * price, 
	...
	__shared__ float xyz2_buf[batch * 4];
	...
	for (int i = blockIdx.x; i < b; i += gridDim.x) {
		...
		float x1, y1, z1, q1, best = -1e9, better = -1e9;
		...
		if (threadIdx.x < thread_per_unass * unass_this_block) {
			...
			x1 = xyz1[(i * n + _unass_id) * 4 + 0];
			y1 = xyz1[(i * n + _unass_id) * 4 + 1];
			z1 = xyz1[(i * n + _unass_id) * 4 + 2];
			q1 = xyz1[(i * n + _unass_id) * 4 + 3];
		}

		for (int k2 = 0; k2 < n; k2 += batch) {
			int end_k = min(n, k2 + batch) - k2;
			for (int j = threadIdx.x; j < end_k * 4; j += blockDim.x) {
				xyz2_buf[j] = xyz2[(i * n + k2) * 4 + j];
			}
			...
			if (_unass_id != -1) {
				...
				{
					float x2 = xyz2_buf[k * 4 + 0] - x1;
					float y2 = xyz2_buf[k * 4 + 1] - y1;
					float z2 = xyz2_buf[k * 4 + 2] - z1;
					float q2 = xyz2_buf[k * 4 + 3] - q1;
					// the coordinates of points should be normalized to [0, 1]
					float d = 4.0 - sqrtf(x2 * x2 + y2 * y2 + z2 * z2 + q2 * q2) - price_buf[k];
					if (d > best) {
						better = best;
						best = d;
						best_i = k + k2;
...
__global__ void CalcDist(int b, int n, float * xyz1, float * xyz2, float * dist, int * assignment) {
	for (int i = blockIdx.x; i < b; i += gridDim.x) {
		...
		float deltax = xyz1[(i * n + j) * 4 + 0] - xyz2[(i * n + k) * 4 + 0];
		float deltay = xyz1[(i * n + j) * 4 + 1] - xyz2[(i * n + k) * 4 + 1];
		float deltaz = xyz1[(i * n + j) * 4 + 2] - xyz2[(i * n + k) * 4 + 2];
		float deltaq = xyz1[(i * n + j) * 4 + 3] - xyz2[(i * n + k) * 4 + 3];
...
__global__ void NmDistanceGradKernel(int b, int n, const float * xyz1, const float * xyz2, const float * grad_dist, const int * idx, float * grad_xyz){
	for (int i = blockIdx.x; i < b; i += gridDim.x) {
		for (int j = threadIdx.x + blockIdx.y * blockDim.x; j < n; j += blockDim.x * gridDim.y) {
			...
			int j2 = idx[i * n + j];
			float x2 = xyz2[(i * n + j2) * 4 + 0];
			float y2 = xyz2[(i * n + j2) * 4 + 1];
			float z2 = xyz2[(i * n + j2) * 4 + 2];
			float q2 = xyz2[(i * n + j2) * 4 + 3];
			float g = grad_dist[i * n + j] * 2;
			atomicAdd(&(grad_xyz[(i * n + j) * 4 + 0]), g * (x1 - x2));
			atomicAdd(&(grad_xyz[(i * n + j) * 4 + 1]), g * (y1 - y2));
			atomicAdd(&(grad_xyz[(i * n + j) * 4 + 2]), g * (z1 - z2));
			atomicAdd(&(grad_xyz[(i * n + j) * 4 + 3]), g * (q1 - q2));
		}
	}
}
